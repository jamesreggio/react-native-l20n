/*! l20n.js v4.0.0-alpha.3 | Apache-2.0 License | github.com/l20n/l20n.js
    This copy was modified to remove dependency upon Node.js APIs. */
...
class ParseContext {
  ...
  getKeyword() {
    ...
    return namespace ? { type: 'kw', ns: namespace, name } : { type: 'kw', name };
  }
...
function resolve(iter) {
  return function step(resume) {
    ...
    const rw = value instanceof ReadWrite ? value : unit(value);
...
class FTLList extends Array {
  toString(ctx) {
    const lf = ctx._memoizeIntlObject(Intl.ListFormat // XXX add this.opts
    );
    const elems = this.map(elem => elem.toString(ctx));
...
var builtins = {
  'NUMBER': ([arg], opts) => new FTLNumber(arg.valueOf(), merge(arg.opts, opts)),
  'PLURAL': ([arg], opts) => new FTLNumber(arg.valueOf(), merge(arg.opts, opts)),
  'DATETIME': ([arg], opts) => new FTLDateTime(arg.valueOf(), merge(arg.opts, opts)),
...
function valuesOf(opts) {
  return Object.keys(opts).reduce((seq, cur) => Object.assign({}, seq, {
...
function* ExternalArgument({ name }) {
  ...
  switch (typeof arg) {
    ...
    default:
      yield tell(new TypeError(`Unsupported external type: ${ name }, ${ typeof arg }`));
      return new FTLNone(name);
  }
}
...
function* Pattern(ptn) {
  ...
  for (let part of ptn) {
    if (typeof part === 'string') {
      ...
      const value = part.length === 1 ? yield* Value(part[0]) : yield* mapValues(part);

      const str = value.toString(ctx);
      if (str.length > MAX_PLACEABLE_LENGTH) {
        yield tell(new RangeError('Too many characters in placeable ' + `(${ str.length }, max allowed is ${ MAX_PLACEABLE_LENGTH })`));
...
class ParseContext$1 {
  ...
  getJunkEntry() {
    ...
    const junk = new AST.JunkEntry(this._source.slice(entityStart, nextEntity));
    return junk;
  }
